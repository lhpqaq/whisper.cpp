...
static __device__ __forceinline__ float seg_sigmoidf(float x) {
    // Use CUDA fast exp implementation. This is materially faster than expf
    // and matches the intent of using cuDNN/PyTorch-style fast math on GPU.
...
static __device__ __forceinline__ float seg_tanhf(float x) {
    // Match the fast-math choice used for sigmoid.
    return tanhf(x);
}
...
static __global__ void k_pyannote_seg_lstm_dir_coop(
    ...
    // Shared cache of the previous hidden state for this block.
    // Each time step requires reading the full H-vector; caching here reduces
    // redundant global loads across threads.
    extern __shared__ float sh_hp[];
    ...
    for (int step = 0; step < T; ++step) {
        ...
        // Cache hp in shared memory (all threads in the block participate).
        for (int k = (int) threadIdx.x; k < H; k += (int) blockDim.x) {
            sh_hp[k] = hp[k];
        }
        __syncthreads();
        ...
        // Ensure all blocks have finished writing hn/cn before any block loads hp for the next step.
        // This is a grid-level barrier (cooperative launch requirement).
        grid.sync();

        // Make sure the shared cache is not read across steps without a refresh.
        __syncthreads();
    }
}

// Warp-per-hidden cooperative recurrence.
//
// Motivation (esp. on T4/sm_75): the recurrent weights `w_hh` are stored column-major (ld = H).
// In the original 1-thread-per-hidden kernel, warp lanes read strided addresses and global loads
// are poorly coalesced. Here we assign one warp to one hidden unit `h` and split the dot-product
// across lanes over `k`, making loads contiguous within the warp.
template <bool REVERSE, typename W_T, typename B_T, int WARPS_PER_BLOCK>
static __global__ void k_pyannote_seg_lstm_dir_coop_warp(
    const int T,
    const int H,
    const float * ih_all_all,    // batched [B, 4H, T] with stride ih_stride
    const int ih_stride,
    ...
    const int h = (int) (blockIdx.x * WARPS_PER_BLOCK + warp_id);
    const bool active = h < H;
    ...
    for (int step = 0; step < T; ++step) {
        ...
        // Cache hp in shared memory for this block.
        for (int k = (int) threadIdx.x; k < H; k += (int) blockDim.x) {
            sh_hp[k] = hp[k];
        }
        __syncthreads();
        ...
        if (active) {
            ...
            const W_T * w_i = w_hh + col_i;
            const W_T * w_f = w_hh + col_f;
            const W_T * w_g = w_hh + col_g;
            const W_T * w_o = w_hh + col_o;

            // Lane-parallel dot products over k. The reduction order differs from the legacy kernel.
            if (H == 128) {
#pragma unroll
                for (int it = 0; it < 4; ++it) {
                    const int k = lane + 32*it;
                    const float hp_k = sh_hp[k];
                    dot_i = fmaf(seg_load_f32<W_T>(w_i, k), hp_k, dot_i);
                    dot_f = fmaf(seg_load_f32<W_T>(w_f, k), hp_k, dot_f);
                    dot_g = fmaf(seg_load_f32<W_T>(w_g, k), hp_k, dot_g);
                    dot_o = fmaf(seg_load_f32<W_T>(w_o, k), hp_k, dot_o);
                }
            } else {
#pragma unroll 1
...
// Warp-per-hidden cooperative recurrence specialized for FP16 recurrent weights.
// Uses half2 vectorization for the H=128 case (the model's fixed hidden size).
template <bool REVERSE, typename B_T, int WARPS_PER_BLOCK>
static __global__ void k_pyannote_seg_lstm_dir_coop_warp_h2(
    const int T,
    const int H,
    const float * ih_all_all,    // batched [B, 4H, T] with stride ih_stride
    const int ih_stride,
    const half  * w_hh,          // [H, 4H] as ggml ne0=H, ne1=4H (column-major ld=H)
    const B_T   * b_ih,          // [4H]
    const B_T   * b_hh,          // [4H]
    float       * dst_all,       // batched [B, 2H, T] with stride dst_stride
    const int dst_stride,
    ...
    for (int step = 0; step < T; ++step) {
        ...
        if (active) {
            ...
            if (H == 128) {
                // Each lane handles 2 elements via half2. Total: 2 iterations * 64 elems = 128.
...
#pragma unroll
                for (int it = 0; it < 2; ++it) {
                    ...
                    const float hp0 = sh_hp[k0];
                    const float hp1 = sh_hp[k1];
...
static void ggml_cuda_pyannote_seg_lstm_custom(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // This is intentionally runtime-tunable because the optimal threads-per-block depends
    // on the GPU and the cost tradeoff between (a) grid.sync() overhead and (b) per-step
    // math throughput.
    //
    // Env override (optional): DIARIZATION_SEG_LSTM_COOP_THREADS in { 8, 16, 32 }.
    // Default is 8 (historically the best on T4 for this specific kernel shape).
    int coop_threads = 8;
    if (const char * env_thr = getenv("DIARIZATION_SEG_LSTM_COOP_THREADS")) {
        const int v = std::atoi(env_thr);
        if (v == 8 || v == 16 || v == 32) {
            coop_threads = v;
        }
    }
    ...
    // Optional alternative cooperative kernel: warp-per-hidden mapping.
    // Enabled via DIARIZATION_SEG_LSTM_COOP_WARP=1 and tuned with DIARIZATION_SEG_LSTM_COOP_WARPS.
    const char * coop_warp_env = getenv("DIARIZATION_SEG_LSTM_COOP_WARP");
    const bool want_coop_warp = (coop_warp_env != nullptr) && (std::strcmp(coop_warp_env, "0") != 0);
        ...
        if (use_coop) {
            ...
            if (want_coop_warp) {
                switch (coop_warps) {
                    case 2:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<false, half, 2>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 4:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<false, half, 4>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 8:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<false, half, 8>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 16:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<false, half, 16>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    default:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                ...
                switch (coop_threads) {
                    case 8:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<false, half, half, 8>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 16:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<false, half, half, 16>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 32:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<false, half, half, 32>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    default:
        ...
        if (use_coop) {
            ...
            if (want_coop_warp) {
                switch (coop_warps) {
                    case 2:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<true, half, 2>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 4:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<true, half, 4>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 8:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<true, half, 8>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 16:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<true, half, 16>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    default:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                ...
                switch (coop_threads) {
                    case 8:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<true, half, half, 8>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 16:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<true, half, half, 16>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 32:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<true, half, half, 32>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    default:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
        ...
        if (use_coop) {
            ...
            if (want_coop_warp) {
                switch (coop_warps) {
                    case 2:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<false, float, 2>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 4:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<false, float, 4>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 8:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<false, float, 8>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 16:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<false, float, 16>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    default:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                ...
                switch (coop_threads) {
                    case 8:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<false, half, float, 8>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 16:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<false, half, float, 16>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 32:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<false, half, float, 32>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    default:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
        ...
        if (use_coop) {
            ...
            if (want_coop_warp) {
                switch (coop_warps) {
                    case 2:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<true, float, 2>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 4:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<true, float, 4>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 8:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<true, float, 8>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 16:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop_warp_h2<true, float, 16>,
                            coop_warp_grid, coop_warp_block, kernel_args2, coop_shmem, stream));
                        break;
                    default:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                ...
                switch (coop_threads) {
                    case 8:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<true, half, float, 8>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 16:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<true, half, float, 16>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    case 32:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
                            (void *) k_pyannote_seg_lstm_dir_coop<true, half, float, 32>,
                            coop_grid, coop_block, kernel_args2, coop_shmem, stream));
                        break;
                    default:
                        CUDA_CHECK(cudaLaunchCooperativeKernel(
