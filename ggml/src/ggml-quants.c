...
void quantize_row_q2_K_ref(const float * restrict x, block_q2_K * restrict y, int64_t k) {
    assert(k % Q2K_K == 0);
    const int nb = k / Q2K_K;

    uint8_t L[Q2K_K];
    uint8_t Laux[16];
    float   weights[16];
    float mins[Q2K_K/16];
    float scales[Q2K_K/16];
    ...
    for (int i = 0; i < nb; i++) {
        ...
        if (max_scale > 0) {
            ...
            for (int j = 0; j < Q2K_K/16; ++j) y[i].scales[j] = 0;
